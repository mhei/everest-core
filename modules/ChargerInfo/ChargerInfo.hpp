...
public:
    ChargerInfo() = delete;
    ChargerInfo(const ModuleInfo& info, std::unique_ptr<charger_informationImplBase> p_charger_information,
                std::unique_ptr<kvsIntf> r_kvs, Conf& config) :
        ModuleBase(info),
        p_charger_information(std::move(p_charger_information)),
        r_kvs(std::move(r_kvs)),
        config(config){};

    const std::unique_ptr<charger_informationImplBase> p_charger_information;
